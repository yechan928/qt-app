...
- `components/BfcacheRefresh.tsx`(2026-08-14 추가) — `(protected)/layout.tsx`에 마운트되는 빈 렌더(`null`) 컴포넌트. `pageshow` 이벤트의 `event.persisted`가 true(=진짜 브라우저 문서 언로드 후 bfcache로 복원, 예: 앱 백그라운드 전환 후 복귀)일 때 `router.refresh()` 호출. **주의**: Next.js 자체 SPA 내비게이션(뒤로가기 포함)은 문서 언로드가 없어서 이 이벤트가 안 불릴 수 있음 — 아멘 등의 실제 원인은 아래 `RefreshOnMount` 참고, 이 컴포넌트는 보조 안전망으로만 유지
- `components/RefreshOnMount.tsx`(2026-08-14 추가, TODO.md 참고) — 마운트될 때마다 무조건 `router.refresh()` 호출. Next.js는 뒤로가기/앞으로가기 탐색에서 `staleTimes` 설정과 무관하게 항상 Client Cache를 재사용하는 게 공식 문서에 명시된 동작이라, 최신 데이터가 중요한 화면(뒤로가기로 자주 재방문됨)에는 이렇게 마운트 시점에 강제로 다시 가져와야 함. 현재 `groups/[id]/page.tsx`(그룹원 목록·아멘 수)와 `posts/[id]/page.tsx`(아멘 상태)에 적용 — 다른 화면에서 같은 증상 보고되면 그 페이지에도 추가
